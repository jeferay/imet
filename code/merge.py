...
class Stack2(nn.Module):
    def __init__(self,num_classes,lr,device,num_epoches,batch_size,threshold,model_name,data_dir,valid_fold,weight_dacay):
        ...
        self.parameter_path='../input/resnext50/resnext50'+str(self.valid_fold)+'.pth'#生成参数的保存路径
...
class Stack1(nn.Module):
    ...
    def train(self,pre_trained = False, min_loss = np.inf):
        ...
        path ='./resnext50'+str(self.valid_fold)+'.pth'
        torch.save(bestmodel,path)
        print('训练结束，总耗时：',(time.time()-start_time)/60)
    ...
    def predict(self):
        ...
        for model_name in ['se_resnext50','se_resnext50','se_resnet50']: #三个模型
            ...
            for fold in range(6):#根据fold选择测试集以及模型参数
                ...
                parameter_path ='../input/se-resnet50/se_resnext50'+str(fold)+'.pth'#根据模型和fold选择模型参数
                if model_name == 'se_resnext50':
                    parameter_path = '../input/se-resnext500/se_resnext50'+str(fold)+'.pth'
                model.load_state_dict(torch.load(parameter_path))
        ...
        np.savetxt('train_stack.csv',preds,delimiter=',')
